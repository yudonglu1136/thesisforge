...
class _AiInsightsPanelState extends State<AiInsightsPanel> {
  ...
  Widget compactResearchDesk(double width) {
    final deskSort = const {'growth', 'quality', 'composite'}.contains(sort)
        ? sort
        : 'composite';
    final ranked = filterAiInsightsCompanies(
      companies,
      group: group,
      sector: sector,
      query: search.text,
    ...
    final selected =
        ranked.where((row) => row['ticker'] == selectedTicker).firstOrNull ??
        companies.where((row) => row['ticker'] == selectedTicker).firstOrNull ??
        ranked.firstOrNull;
    final list = panel([
      Wrap(
        alignment: WrapAlignment.spaceBetween,
        ...
        children: [
          title(
            ...
            '公司研究榜单',
            hintEn: 'Fixed peer scores · filters never recalculate rank',
            hintZh: '固定同组分数 · 筛选不会重算名次',
          ),
          TextButton(
            onPressed: () => setTab('companies'),
            child: Text(w('Open full table →', '打开完整表格 →')),
          ),
        ],
      ),
      Wrap(
        ...
        children: [
          for (final item in [
            ('growth', w('Growth', '增长')),
            ('quality', w('Quality', '质量')),
            ('composite', w('Composite', '综合')),
          ])
            ChoiceChip(
              label: Text(item.$2),
              selected: deskSort == item.$1,
              onSelected: (_) {
      ...
      for (final row in ranked.take(8))
        Semantics(
          ...
          label: '${row['ticker']} ${score(row['${deskSort}Score'])}',
          child: InkWell(
            key: ValueKey('ai-company-${row['ticker']}'),
            onTap: () {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: row['ticker'] == selectedTicker
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    text(asMap(row['rank'])[deskSort], '—'),
                    style: style(11, true, p.muted),
                  ),
                  const SizedBox(width: 10),
                  SizedBox(
                    width: 44,
                    child: Text(
                      score(row['${deskSort}Score']),
    ...
    final detail = selected == null
        ? panel([
            caption('Select a company to inspect its evidence.', '请选择公司查看证据。'),
          ])
        : selectedCompanyPreview(selected);
    if (width >= 960) {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget selectedCompanyPreview(Map<String, dynamic> row) {
    final ticker = text(row['ticker']);
    return panel([
      Text(
        w('Selected company evidence', '选中公司证据'),
        style: style(11, true, p.accent),
      ),
      const SizedBox(height: 8),
      Row(
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(ticker, style: style(25, true)),
                Text(text(row['name']), style: style(13, false, p.muted)),
              ],
            ),
          ),
          Checkbox(
            value: tickers.contains(ticker),
            semanticLabel: w('Add $ticker to comparison', '将 $ticker 加入对比'),
            onChanged: (_) => toggleCompare(ticker),
          ),
        ],
      ),
      const SizedBox(height: 14),
      Wrap(
        ...
        runSpacing: 12,
        children: [
          previewMetric(w('Revenue', '收入'), aiInsightsAmount(row['revenue'])),
          previewMetric(
            'YoY',
            aiInsightsPercent(row['revenueYoY'], signed: true),
          ),
          previewMetric(
            'QoQ',
            aiInsightsPercent(row['revenueQoQ'], signed: true),
          ),
          previewMetric(
            w('Op. margin · TTM', '经营利润率 · TTM'),
            aiInsightsPercent(row['ttmOperatingMargin']),
          ),
          previewMetric(
            w('FCF margin · TTM', 'FCF 率 · TTM'),
            aiInsightsPercent(row['ttmFcfMargin']),
          ),
          previewMetric(w('Composite', '综合分'), score(row['compositeScore'])),
        ],
      ),
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              '${text(row['quarter'], quarter)} · ${text(row['reportperiod'], '—')}',
              style: style(12, true),
            ),
            const SizedBox(height: 4),
            Text(
              w(
                'Available ${text(row['datekey'], '—')} · ${statusLabel(row['status'])}',
                '可得日 ${text(row['datekey'], '—')} · ${statusLabel(row['status'])}',
              ),
      ...
      Wrap(
        ...
        children: [
          FilledButton.icon(
            key: const ValueKey('ai-open-company-evidence'),
            onPressed: () => showCompany(row),
            icon: const Icon(Icons.fact_check_outlined, size: 17),
            label: Text(w('Inspect sources & scoring', '查看来源与评分')),
          ),
          OutlinedButton.icon(
            key: const ValueKey('ai-open-research'),
            onPressed: () => widget.onCompany(ticker, 'evidence'),
            icon: const Icon(Icons.open_in_new, size: 17),
            label: Text(w('Open Research', '进入 Research')),
          ),
        ],
      ),
    ]);
  ...
  Widget previewMetric(String label, String value) => SizedBox(
    width: 128,
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: style(10, false, p.muted)),
        const SizedBox(height: 2),
        Text(value, style: style(15, true)),
      ],
    ),
  );
  ...
  Widget overview(double width) => Column(
    ...
    children: [
      ...
      Material(
        ...
        child: ExpansionTile(
          key: const ValueKey('ai-insights-market-context'),
          initiallyExpanded: false,
          tilePadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 6),
          childrenPadding: const EdgeInsets.fromLTRB(20, 0, 20, 20),
          title: Text(
            w('Market context & detailed trends', '市场背景与详细趋势'),
            style: style(17, true),
          ),
          subtitle: Text(
            w(
              'Quarterly charts, sector heatmap, contributors and accessible data tables',
              '季度图表、行业热力图、增量贡献与可访问数据表',
            ),
            style: style(12, false, p.muted),
          ),
          children: [marketContextContent(width)],
        ),
      ),
    ],
  );

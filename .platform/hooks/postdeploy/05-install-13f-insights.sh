...
if (
    manifest.get("version") not in {"institutional-13f-artifact-v3", "institutional-13f-artifact-v5", "institutional-13f-artifact-v6", "institutional-13f-artifact-v7"}
...
try:
    ...
    if manifest.get("version") in {"institutional-13f-artifact-v6", "institutional-13f-artifact-v7"}:
        expected_tables.update({
            "institutional_13f_active_snapshots_v1",
            "institutional_13f_active_details_v1",
        })
    if manifest.get("version") == "institutional-13f-artifact-v7":
        expected_tables.add("institutional_13f_active_sectors_v1")
    ...
    active_rows = None
    active_detail_rows = None
    if manifest.get("version") in {"institutional-13f-artifact-v6", "institutional-13f-artifact-v7"}:
        active_rows = connection.execute("SELECT count(*) FROM institutional_13f_active_snapshots_v1").fetchone()[0]
        active_detail_rows = connection.execute("SELECT count(*) FROM institutional_13f_active_details_v1").fetchone()[0]
    if manifest.get("version") == "institutional-13f-artifact-v7":
        sectors = connection.execute("SELECT count(*) FROM institutional_13f_active_sectors_v1").fetchone()[0]
        if sectors < 1 or sectors != manifest.get("sectorRows"):
            raise SystemExit("13f_sector_rows_invalid")
    if (
        ...
        or (
            manifest.get("version") in {"institutional-13f-artifact-v6", "institutional-13f-artifact-v7"}
            and (
                active_rows != manifest.get("activeRows") or active_rows < 1
                or active_detail_rows != manifest.get("activeDetailRows") or active_detail_rows < 1
            )
        )
    ):
